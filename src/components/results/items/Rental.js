...
import { ReactComponent as RentalIcon } from "../../../assets/images/rentalIcon.svg";
...
const Rental = ({
  ...
  async function getImageUrl() {
    const url = await storageRef
      .child(`products_images/${featureImage}`)
      .getDownloadURL();
    setFeatured(url);
  }
  ...
  return (
    <div className="rental-item" onClick={() => history.push(`${path}/${id}`)}>
      <div className="rental-top">
        <div className="rental-timer">
          <RentalIcon /> <span>{images.length}</span>
        </div>
      ...
      <div className="rental-info">
        <div className="info-heading">
          <div className="rental-price">
            ...
            <span> CFA / {paymentMode === "daily" ? "jour" : "mois"}</span>
          </div>
          <ul className="details">
            <li>
              {bedrooms} <abbr>ch</abbr>
            </li>
            <li>
              {bathrooms} <abbr>dc</abbr>
            </li>
          </ul>
        </div>
